...
export function findLedSync(brightness,fps,settings={}){
  const searchEnd=settings.searchEnd??10,midFirst=settings.midFirst??2.5,midInterval=settings.midInterval??5,tol=settings.midTolerance??.40,accept=settings.midCorrelation??.4;
...
export function compareVideoRwlog(rwlog,tracking,led,minimumMidCorrelation=.4){
  const all=rwlog.rows,logs=measurementRows(all),rwAnchors=extractRwlogAnchors(all);
  if(led.mid_blinks.length!==6)throw new Error(`動画MID同期は6個必要です (検出 ${led.mid_blinks.length})`);
  const videoAnchors=[led.start.measurement_start_video_s,...led.mid_blinks.map(x=>x.pattern_start_video_s),led.end.measurement_end_video_s];
  if(videoAnchors.some((v,i)=>i>0&&v<=videoAnchors[i-1]))throw new Error('動画同期アンカーが時系列順ではありません');
  const startEndScale=(videoAnchors[videoAnchors.length-1]-videoAnchors[0])/(rwAnchors[rwAnchors.length-1]-rwAnchors[0]);
  const startEndOffset=videoAnchors[0]-startEndScale*rwAnchors[0];
  const predicted=rwAnchors.map(t=>startEndOffset+startEndScale*t);
  const allResidualMs=videoAnchors.map((v,i)=>(v-predicted[i])*1000);
  const midDiagnostics=led.mid_blinks.map((m,i)=>({mid_index:i+1,rwlog_time_s:rwAnchors[i+1],video_time_s:videoAnchors[i+1],predicted_video_time_s:predicted[i+1],residual_ms:allResidualMs[i+1],correlation:m.template_correlation,accepted:!!m.accepted&&Number.isFinite(m.template_correlation)&&m.template_correlation>=minimumMidCorrelation}));
  const midResidualMs=midDiagnostics.map(x=>x.residual_ms).filter(Number.isFinite),acceptedMidResidualMs=midDiagnostics.filter(x=>x.accepted).map(x=>x.residual_ms).filter(Number.isFinite),segmentScales=videoAnchors.slice(1).map((v,i)=>(v-videoAnchors[i])/(rwAnchors[i+1]-rwAnchors[i]));
  const vr=tracking.rows.filter(r=>r.in_measurement_window&&r.valid&&Number.isFinite(r.video_white_angle_deg)),vAbs=vr.map(r=>r.video_time_s),vAngle=vr.map(r=>r.video_white_angle_deg),logT=logs.map(r=>r.t_test_ms/1000),cols=pitchColumns(logs),series={};
  for(const c of cols){const z=t0Reference(all,c);series[c]=logs.map(r=>r[c]-z);}
  function build(mode){const vt=vAbs.map(t=>mode==='primary'?(t-startEndOffset)/startEndScale:interp(t,videoAnchors,rwAnchors)),videoAtLog=logT.map(t=>interp(t,vt,vAngle)),videoTimeAtLog=logT.map(t=>mode==='primary'?startEndOffset+startEndScale*t:interp(t,rwAnchors,videoAnchors)),m={};for(const c of cols)m[c]=metrics(series[c].map((v,i)=>v-videoAtLog[i]));const rows=logT.map((t,i)=>{const row={t_test_s:t,video_time_s:videoTimeAtLog[i],video_white_angle_deg:videoAtLog[i]};for(const c of cols){row[`${c}_zeroed`]=series[c][i];row[`${c}_error_deg`]=series[c][i]-videoAtLog[i];}return row;});return{rows,metrics:m,videoAtLog,videoTimeAtLog};}
  const primary=build('primary'),piecewise=build('piecewise');
  return{logT,series,columns:cols,primary,global:primary,piecewise,sync:{primary_mapping:'start_end_affine',mid_sync_role:'diagnostic_only',rwlog_anchors_s:rwAnchors,video_anchors_s:videoAnchors,start_end_scale:startEndScale,start_end_offset_s:startEndOffset,all_anchor_residual_ms:allResidualMs,mid_residual_ms:midResidualMs,mid_residual_rmse_ms:midResidualMs.length?Math.sqrt(mean(midResidualMs.map(x=>x*x))):NaN,mid_residual_max_abs_ms:midResidualMs.length?Math.max(...midResidualMs.map(Math.abs)):NaN,accepted_mid_residual_rmse_ms:acceptedMidResidualMs.length?Math.sqrt(mean(acceptedMidResidualMs.map(x=>x*x))):NaN,mid_accepted_count:midDiagnostics.filter(x=>x.accepted).length,mid_diagnostics:midDiagnostics,segment_scales:segmentScales,mid_correlations:led.mid_blinks.map(x=>x.template_correlation),minimum_mid_correlation:minimumMidCorrelation}};
}
